...
function addBorderObstacles() {
  for (let x = 0; x < COLS; x++) {
    obstacles.push(new Obstacle(x * CELL_SIZE, 0, CELL_SIZE))
    obstacles.push(new Obstacle(x * CELL_SIZE, (ROWS - 1) * CELL_SIZE, CELL_SIZE))
  }
  for (let y = 0; y < ROWS; y++) {
    obstacles.push(new Obstacle(0, y * CELL_SIZE, CELL_SIZE))
    obstacles.push(new Obstacle((COLS - 1) * CELL_SIZE, y * CELL_SIZE, CELL_SIZE))
  }
}
...
canvas.addEventListener('click', e => {
  ...
  if (mode === 'obstacle') {
    ...
    obstacles = obstacles.filter(o => {
      const isBorder = o.x === 0 || o.y === 0 || o.x === (COLS-1)*CELL_SIZE || o.y === (ROWS-1)*CELL_SIZE
      return isBorder || !(o.x===x && o.y===y)
    })
  } else if (mode === 'target') {
...
document.getElementById('startSim').onclick = () => {
  if (!car) {
    car = new Car(ctx, bg, 0.5, 0, obstacles, { startX: CELL_SIZE * 2, startY: CELL_SIZE * 2 })
    drawAll()
...
function generateMaze(cols, rows) {
  const STEP = 4
  const cCols = Math.floor(cols / STEP)
  const cRows = Math.floor(rows / STEP)
  const maze = Array.from({ length: cRows }, () => Array(cCols).fill(1))
  const stack = []
  const start = { x: 1, y: 1 }
  ...
  while (stack.length) {
    ...
    const neighbors = dirs.map(d=>({x:cur.x+d[0]*2,y:cur.y+d[1]*2}))
      .filter(n=>n.x>0&&n.x<cCols-1&&n.y>0&&n.y<cRows-1&&maze[n.y][n.x]===1)
    if (neighbors.length) {
      ...
      if (maze[y][x] === 1) {
        for (let yy = 0; yy < STEP; yy++) {
          for (let xx = 0; xx < STEP; xx++) {
            obs.push(new Obstacle((x*STEP+xx)*CELL_SIZE, (y*STEP+yy)*CELL_SIZE, CELL_SIZE))
          }
        }
      }
    }
